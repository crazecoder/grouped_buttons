...
class _CheckboxGroupState extends State<CheckboxGroup> {
  ...
  Widget build(BuildContext context) {
    ...
    for(int i = 0; i < widget.labels.length; i++){
      ...
      Checkbox cb = Checkbox(
                      ...
                      activeColor: widget.activeColor ?? Theme.of(context).toggleButtonsTheme.selectedColor,
  ...
  void onChanged(bool? isChecked, int i){
    ...
    if(mounted){
      setState(() {
        if(isChecked!=true && isAlreadyContained){
          _selected.remove(widget.labels.elementAt(i));
        }else if(isChecked==true && !isAlreadyContained){
          _selected.add(widget.labels.elementAt(i));
        }

        if(widget.onChange != null) widget.onChange!(isChecked==true, widget.labels.elementAt(i), i);
